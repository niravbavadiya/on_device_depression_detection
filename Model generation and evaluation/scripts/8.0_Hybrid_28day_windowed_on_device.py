...
# --- TensorFlow Lite Runtime Conversion (LiteRT) ---
print("\n--- Converting to TensorFlow Lite Runtime (LiteRT) ---")

# 1) create converter from your final Keras model
converter = tflite.TFLiteConverter.from_keras_model(final_model)

# 2) allow SELECT_TF_OPS to keep dynamic list/TensorArray support
converter.target_spec.supported_ops = [
    tflite.OpsSet.TFLITE_BUILTINS,   # default TFLite kernels
    tflite.OpsSet.SELECT_TF_OPS      # fallback to TF kernels when needed
]

# 3) disable the lowering pass that trips on dynamic shapes
converter._experimental_lower_tensor_list_ops = False

# 4) (optional) still apply default post-training optimizations
converter.optimizations = [tflite.Optimize.DEFAULT]

# 5) convert and write out
lite_rt_model = converter.convert()
lite_rt_path = RUN_DIR / "model_litert.tflite"
with open(lite_rt_path, "wb") as f:
    f.write(lite_rt_model)

print(f"✅ LiteRT model saved to: {lite_rt_path}")
...
interpreter = tflite_rt(model_path=str(lite_rt_path))
...
ax_cm.set_title("LiteRT Model Confusion Matrix")
plt.tight_layout()
fig_cm.savefig(get_next_filename("litert_confusion_matrix"))
plt.close(fig_cm)
...
train_acc = final_model.evaluate(X_train, y_train, verbose=0)[1]
...
fig.savefig(get_next_filename("confusion_matrix"))
plt.close(fig)
...
fig.savefig(get_next_filename("roc_curve"))
plt.close(fig)
