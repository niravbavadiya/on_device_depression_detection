...
from tensorflow.keras.layers import LSTM, Dense, Dropout, BatchNormalization, Conv1D, GlobalMaxPooling1D
...
def generate_fixed_windows(sensor_df, survey_df, window_size=28):
    ...
    for pid in survey_df["pid"].unique():
        ...
        for _, row in svy_pid.iterrows():
            ...
            missing_days = features.isna().all(axis=1).sum()
...
for dataset in DATASETS:
    ...
    output_path = Path(r"C:\Users\Nirav Bavadiya\Documents\study\Masters\Thesis\My work\full_dataset_" + dataset.name + ".csv")
    merged.to_csv(output_path, index=False)
    print(f"✅ Saved full merged data for {dataset.name} to: {output_path}")
    ...
    X_windows, y_labels, quality_df = generate_fixed_windows(merged, survey)
    quality_df.to_csv(RUN_DIR / f"window_quality_{dataset.name}.csv", index=False)
    ...
    for train_idx, test_idx in splitter.split(X_windows, y_labels):
        X_train_all.extend(X_windows[train_idx])
        y_train_all.extend(y_labels[train_idx])
        X_test_all.extend(X_windows[test_idx])
        y_test_all.extend(y_labels[test_idx])
...
model.save(RUN_DIR / "trained_28days_windowed.keras")
