...
# --- Helper: count parameters & sparsity (for pruning effect) ---
...
pruned_keras_path = RUN_DIR / "pruned_model.keras"
final_model.save(pruned_keras_path)
print("✅ Pruning wrappers stripped and Keras model saved.")

# --- Report pruning effect: sparsity & effective size reduction ---
total_params, nonzero_params, sparsity = count_nonzero_params(final_model)
print("\n--- Unstructured Pruning Summary ---")
print(f"Total trainable parameters (dense baseline): {total_params}")
print(f"Non-zero parameters after pruning:          {nonzero_params}")
print(f"Sparsity after pruning:                     {sparsity * 100:.2f}%")
print(f"Effective parameter reduction (vs dense):   {total_params / nonzero_params:.2f}x smaller")

# On-disk Keras model size
pruned_keras_size_mb = os.path.getsize(pruned_keras_path) / (1024 * 1024)
print(f"Pruned Keras model file size:               {pruned_keras_size_mb:.2f} MB")
...
# save q-aware model
q_aware_model_path = RUN_DIR / "q_aware_model.keras"
...
# TFLite file size
tflite_size_mb = os.path.getsize(q_lite_path) / (1024 * 1024)
print(f"LiteRT (QAT) TFLite model file size:         {tflite_size_mb:.2f} MB")
